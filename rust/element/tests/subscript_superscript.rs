...
/// Corpus discrepancy: `writting_collection.org` (@161091, @165934).
///
/// A bare (brace-less) subscript's content must include Indic combining
/// marks such as the Bengali virama `্` (U+09CD).  Emacs' `[[:alnum:]]`
/// treats virama as a word constituent, so `_বোস্টন` is one subscript.
///
/// org-rs uses `char::is_alphanumeric()` to scan bare sub/superscript
/// content; that predicate is FALSE for the virama (a non-spacing mark
/// that is not in Unicode `Other_Alphabetic`), so the subscript is cut
/// short at `_বোস`, leaving `্টন` as stray plain text.
#[test]
fn bare_subscript_includes_bengali_virama() {
    let input = "জেট_বোস্টন test\n";
    let bump = Bump::new();
    let mut parser = Parser::new(input, ParseGranularity::Object, DefaultEnvironment, &bump);
    let (arena, root) = parser.parse_buffer();
    let scripts = find_script(&arena, root);
    assert!(
        !scripts.is_empty(),
        "expected a Subscript node for '_বোস্টন'"
    );
    let node = &arena[scripts[0]];
    let loc = node.location;
    let text = &input[loc.start..loc.end - node.post_blank];
    assert_eq!(
        text, "_বোস্টন",
        "bare subscript must span '_বোস্টন' (incl. virama '্'), got {:?} — \
         is_alphanumeric() drops the Bengali virama and cuts it short",
        text
    );
}
