...
/// Byte-level equivalent of the former `REGEX_DRAWER` regex: returns `true` when
/// `line` (a single line) matches the drawer header pattern `[ \t]*:NAME:[ \t]*`
/// where NAME is one or more word / `-` / `_` chars.  The line may include a
/// trailing newline (matched as trailing whitespace).
/// The caller must additionally verify that the name is not `END` — that check is
/// left to the drawer-parser logic.
///
/// Does a single combined scan (validates name bytes while finding the closing
/// colon) to avoid iterating non-matching lines twice.
#[inline]
pub fn is_drawer_header_line(line: &str) -> bool {
    let bytes = line.as_bytes();
    let s = match bytes.iter().position(|&b| b != b' ' && b != b'\t') {
        Some(i) => i,
        None => return false,
    };
    ...
    // Scan for closing colon, validating name bytes in one pass.
    let mut pos = s + 1;
    loop {
        match bytes.get(pos) {
            None | Some(b'\n') => return false,
...
/// Extract the drawer name from a line that is known (via [`is_drawer_header_line`])
/// to be a valid drawer header.  Returns `None` if the line is not a drawer header.
#[inline]
pub fn extract_drawer_name(line: &str) -> Option<&str> {
    let bytes = line.as_bytes();
    let s = bytes.iter().position(|&b| b != b' ' && b != b'\t')?;
    ...
    let name_end = bytes[name_start..]
        ...
        .position(|&b| b == b':')
        .map(|i| name_start + i)?;
    // Safety: caller should have validated via is_drawer_header_line that the
    // name is ASCII alphanumeric / `-` / `_`, so from_utf8 is safe.
    Some(unsafe { std::str::from_utf8_unchecked(&bytes[name_start..name_end]) })
}
...
fn is_property_drawer_allowed(mode: ParserMode, input: &str, pos: usize) -> bool {
    use ParserMode::*;
    match mode {
        PropertyDrawer | FirstSection | TopComment => true,
        Planning | Nil => {
            // Emacs (`org-element--current-element`, planning mode) allows a
            // property drawer only when the *immediately* preceding line begins
            // with `*` — i.e. `(eq ?* (char-after (line-beginning-position 0)))`.
            // It does NOT skip blank or comment lines: a blank line between the
            // headline and `:PROPERTIES:` makes it a regular drawer.
            let bytes = input.as_bytes();
            let cur_nl = match memrchr(b'\n', &bytes[..pos]) {
                None => return true, // no preceding line (buffer start)
                Some(nl) => nl,
            };
            let prev_start = memrchr(b'\n', &bytes[..cur_nl]).map(|i| i + 1).unwrap_or(0);
            let prev = &bytes[prev_start..cur_nl];

            // Emacs inspects the first character of the previous line without
            // skipping leading whitespace.
            match prev.first() {
                Some(b'*') => {
                    // Exclude inlinetask END lines (15+ `*` + whitespace + END):
                    // those terminate an inlinetask, so an element precedes here.
                    let stars = prev.iter().take_while(|&&b| b == b'*').count();
                    if stars >= 15 {
                        let after = &prev[stars..];
                        if let Some(off) = after.iter().position(|&b| b != b' ' && b != b'\t') {
                            let rest = &after[off..];
                            if rest.len() >= 3
                                && rest[..3].eq_ignore_ascii_case(b"END")
                                && rest[3..].iter().all(|&b| b == b' ' || b == b'\t')
                            {
                                return false; // inlinetask END → deny
                            }
                        }
                    }
                    true // real headline → allow
                }
                _ => false, // blank / comment / text → regular drawer
            }
        }
        _ => false,
    }
...
impl<'a, 'b, Environment: crate::environment::Environment> Parser<'a, 'b, Environment> {
    ...
    pub fn drawer_parser(&mut self, element_span: ElementSpan<'a, 'b>, mode: ParserMode) -> NodeId {
        ...
        // When affiliated keywords precede the drawer, span.start points to
        // the first keyword, not the drawer header.  The name is extracted from
        // content_start using a byte-level scan (fast, no regex engine).
        let line_end = memchr(b'\n', &self.input.as_bytes()[content_start..])
            .map_or(self.input.len(), |i| content_start + i);
        let name = match extract_drawer_name(&self.input[content_start..line_end]) {
            Some(n) => n,
            None => {
                return self
                    .arena
