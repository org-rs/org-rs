...
6. If you are using =make repro= for performance testing, be aware
   that =make repro= activates additional performance-hungry debugging
   by default.  You can use =make repro REPRO_DEBUG=no= to disable it
   and get more clean profile
...
(defun my-auto-exclude-fn (tag)
  (when (cond ((string= tag "net")
              ...
              ((member tag '("errand" "call"))
               (let ((hr (decoded-time-hour (decode-time))))
                 (or (< hr 8) (> hr 21)))))
    (concat "-" tag)))
...
- =LATEX_CLASS_OPTIONS= ::
  ...
  document class. Square brackets around the class options are
  optional.  If missing, they will be added by Org.
...
them, follow LaTeX conventions.  Square brackets around them are
optional:
...
~org-latex-descriptive-environment~ or keyword
~LATEX_DESCRIPTIVE_ENV~.  You may need to do so, if you mix plain list
items with descriptive items like in:
...
To always use environment ~itemize~ in this case, add this to your
configuration
...
You can also add the desired value to your file or directory-local
variables to set it on a case-by-case basis.

If you don't need all description lists to be adjusted, you can also
use
...
work with LaTeX export and BibTeX bibliographies, or with more generic
formats described using [[https://citationstyles.org/][Citation Style
Language]].  The default citation processor is =basic= - it works with
arbitrary export formats and recognizes both BibTeX and CSL
bibliographies.  More citation processors are distributed as Emacs
packages.
...
- One can also specify a stylistic variation for the citations by
  ...
  colon; this usually makes sense only for the author-year
  styles.  Additionally, one can insert a =/= and a variant name after
  the style.
...
variants are detailed below.  The examples illustrate default results
for the =csl= processor except for styles only supported by =basic=.
Citation styles and variants can be specified by their full name
(e.g., =author/bare=) or shortcut (e.g., =a/b=).
...
- =author= (=a=) :: Provide the author names only.\\
  Variants: =bare= (=b=), =caps= (=c=), =full= (=f=), and all their
  combinations.\\
  Support: =basic= (only =c=), =csl= (all variants), =natbib= (=c=,
  =f=), =biblatex= (=c=, =f=, =cf=).

  | =[cite/author:@dewaal06 p. 5]= | (de Waal et al.) |
  | =[cite/a:@dewaal06 p. 5]=      | (de Waal et al.) |
...
- =year= (=y=) :: Provide the year only, like =noauthor=, but also suppress
  ...
  | =[cite/year:@dewaal06, p. 5]= | (2006) |
  | =[cite/y:@dewaal06, p. 5]=    | (2006) |
...
- =text= (=t=) :: Provide an author-date citation with the author
  ...
  | =[cite/text:@dewaal06 p. 5]= | de Waal et al. (2006, 5)           |
  | =[cite/t/cf:@dewaal06 p. 5]= | De Waal, Li, Wu, and Roy (2006, 5) |

- =title= (=ti=) :: Provide the title only.\\
  Variant: =bare=.\\
  Support: =csl=.

  | =[cite/title:@dewaal06, p. 5]= | ("Unicorn Conservation")       |
...
- =locators= (=l=) :: Provide the locator(s) only, e.g., the page
  ...
  | =[cite/l:@dewaal06 sec. 2]=      | (sec. 2) |

- =numeric= (=nb=) :: Provide the citation number.\\
  Variant: none.\\
  Support: =basic=.[fn:numeric:Other processors than the =basic= one
  can also make numeric- or note-style citations, but this is done by
  using an appropriate style file, rather than adding style variants
  to the individual citations.  A numeric style is the default for the
  =bibtex= exporter.]

  | =[cite/numeric:@dewaal06 p. 5]= | (1 p. 5) |
  | =[cite/n:@dewaal06 p. 5]=       | (1 p. 5) |

- =note= (=ft=) :: Provide a footnote.\\
  Variant:\\
  Support: =basic=.

  | =[cite/note:@dewaal06 p. 5]= | [1] |
  | =[cite/n:@dewaal06 p. 5]=    | [1] |
...
- =full= (=f=) :: List all author names instead of replacing them with
  ...
  | =[cite//full:@dewaal06 p. 5]= | (de Waal, Li, Wu, and Roy 2006, 5) |
  | =[cite//f:@dewaal06 p. 5]=    | (de Waal, Li, Wu, and Roy 2006, 5) |

- =bare-full= (=bf=) :: Combine =bare= and =full=.

  | =[cite//bare-full:@dewaal06 p. 5]= | de Waal, Li, Wu, and Roy 2006, 5 |
  | =[cite//bf:@dewaal06 p. 5]=        | de Waal, Li, Wu, and Roy 2006, 5 |

- =caps-full= (=cf=) ::  Combine =caps= and =full=.

  | =[cite//caps-full:@dewaal06 p. 5]= | (De Waal, Li, Wu, and Roy 2006, 5) |
  | =[cite//cf:@dewaal06 p. 5]=        | (De Waal, Li, Wu, and Roy 2006, 5) |

- =bare-caps-full= (=bcf=) :: Combine =bare=, =caps=, and =full=.

  | =[cite//bare-caps-full:@dewaal06 p. 5]= | De Waal, Li, Wu, and Roy 2006, 5 |
  | =[cite//bcf:@dewaal06 p. 5]=            | De Waal, Li, Wu, and Roy 2006, 5 |

When =style= is not specified, one of the two default styles is used:
...
- or, if =CITE_EXPORT= is not set, the default =nil= style
...
the final output format, the =bibtex=, =natbib=, and =biblatex=
processors output corresponding Bib(La)TeX cite commands for further
processing.  For example, the =natbib= processor renders
=[cite/author:@Jones]= as ~\citeauthor{Jones}~.  With the =biblatex=
processor the resulting commands can be controlled by customizing
`org-cite-biblatex-styles'.  Results will depend on the style as well
as the engine used (bibtex or biber), and will vary between
processors.
...
(other processors will render them verbatim).  A locator is a part of
the citation suffix that starts with a locator term and ends with the
last comma or digit in the suffix, whichever comes last.  Locator
terms include "p.", "page", and "chap."[fn::The CSL locator terms are
...
#+vindex: org-tangle-with-archived-trees
By default, =org-babel-tangle= skips source blocks located in archived
and commented subtrees.  The variable ~org-tangle-with-archived-trees~
controls whether source blocks in archived subtrees are included
during tangling.  When this variable is nil (the default), such blocks
are ignored.  Source blocks in commented subtrees are always excluded
from tangling and are not affected by this option.
